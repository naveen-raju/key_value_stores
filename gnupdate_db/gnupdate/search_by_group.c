...
PmStatus
dbFindByGroup(PmDatabase *db, const char *group, PmMatches *matches)
{
	DbMatchData *data;
	DbData *dbData;
	BTree *tree;
	...
	offset = btreeSearch(dbData->groupsIndex->mainTree, group);

	if (offset == 0)
		return PM_FAILED; /* Not found. */

	/* The group tree was found. Open it. */
	tree = btreeOpen(dbData->groupsIndex, offset);
