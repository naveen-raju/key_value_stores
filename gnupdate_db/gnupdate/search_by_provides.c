/**
 * @file search_by_provides.c Search by provided dependency support.
 *
...
PmStatus
dbFindByProvides(PmDatabase *db, const char *provides, PmMatches *matches)
{
	DbMatchData *data;
	DbData *dbData;
	BTree *tree;
	...
	offset = btreeSearch(dbData->provDepsIndex->mainTree, provides);

	if (offset == 0)
		return PM_FAILED; /* Not found. */

	/* The group tree was found. Open it. */
	tree = btreeOpen(dbData->provDepsIndex, offset);
